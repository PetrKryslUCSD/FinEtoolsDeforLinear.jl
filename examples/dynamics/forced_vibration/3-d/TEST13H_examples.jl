...
function TEST13H_hva_ray()
    ...
    U_f = zeros(Complex{Float64}, nfreedofs(u), length(frequencies))
...
function TEST13H_hva_str()
    ...
    # 9.483, 12.133, 12.133, 15.468, 15.468 [Hz].
    # 
    # In this benchmark the damping is structural rather than Rayleigh.

    println("""
    Homogeneous square plate, simply-supported on the circumference from
    the test 13 from NAFEMS “Selected Benchmarks for Forced Vibration,” R0016, March 1993.
    The nonzero benchmark frequencies are (in hertz): 2.377, 5.961, 5.961,
    9.483, 12.133, 12.133, 15.468, 15.468 [Hz]. Structural damping.
    """)
    ...
    for k in eachindex(frequencies)
        frequency = frequencies[k]
        omega = 2 * pi * frequency
        U_f[:, k] = (-omega^2 * M_ff + 1im * C_ff + K_ff) \ F_f
    end
...
function allrun()
    println("#####################################################")
    println("# TEST13H_hva_ray ")
    TEST13H_hva_ray()
    println("#####################################################")
    println("# TEST13H_hva_str ")
    TEST13H_hva_str()
    return true
end # function allrun
