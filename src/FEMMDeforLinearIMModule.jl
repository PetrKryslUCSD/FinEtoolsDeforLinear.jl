...
"""
function associategeometry!(self::F,  geom::NodalField{GFT}) where {F<:FEMMDeforLinearIMH8, GFT}
...
function stiffness(self::FEMMDeforLinearIMH8, assembler::A,
    ...
    for i = eachindex(fes) # Loop over elements
...
function stiffness(self::FEMMDeforLinearIMH8, geom::NodalField{GFT},  u::NodalField{UFT}) where {GFT<:Number, UFT<:Number}
