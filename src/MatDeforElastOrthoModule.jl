...
"""
struct  MatDeforElastOrtho{MR<:AbstractDeforModelRed, FT, MTAN<:Function, MUPD<:Function, MTHS<:Function} <: AbstractMatDeforLinearElastic
    mr::Type{MR}
...
function MatDeforElastOrtho(mr::Type{DeforModelRed3D}, args::NTuple{13, FT}) where FT
	...
	function tangentmoduli3d!(self::MatDeforElastOrtho,  D::Matrix{FT},  t::FT, dt::FT, loc::Matrix{FT}, label::Int)
	    copyto!(D,self.D);
	    return D
	end
	function update3d!(self::MatDeforElastOrtho,  stress::Vector{FT}, output::Vector{FT},  strain::Vector{FT}, thstrain::Vector{FT}=zeros(6), t::FT= 0.0, dt::FT= 0.0,  loc::Matrix{FT}=zeros(3,1), label::Int=0, quantity=:nothing)
...
function MatDeforElastOrtho(mr::Type{DeforModelRed2DStress}, args::NTuple{13, FT}) where FT
    mass_density, E1, E2, E3, nu12, nu13, nu23, G12, G13, G23, CTE1, CTE2, CTE3 = args
	function tangentmoduli2dstrs!(self::MatDeforElastOrtho,  D::Matrix{FT},  t::FT, dt::FT, loc::Matrix{FT}, label::Int)
	...
	function update2dstrs!(self::MatDeforElastOrtho, stress::Vector{FT},  output::Vector{FT},  strain::Vector{FT}, thstrain::Vector{FT}=zeros(3), t::FT= 0.0, dt::FT= 0.0,  loc::Matrix{FT}=zeros(3,1), label::Int=0, quantity=:nothing)
		@assert length(stress) == nstressstrain(self.mr)
		D = zeros(3, 3)
...
function MatDeforElastOrtho(mr::Type{DeforModelRed2DStrain}, args::NTuple{13, FT}) where FT
    mass_density, E1, E2, E3, nu12, nu13, nu23, G12, G13, G23, CTE1, CTE2, CTE3 = args
	function tangentmoduli2dstrn!(self::MatDeforElastOrtho, D::Matrix{FT},  t::FT, dt::FT, loc::Matrix{FT}, label::Int)
		ix = [1, 2, 4];
		for i = 1:length(ix)
			for j = 1:length(ix)
	...
	function update2dstrn!(self::MatDeforElastOrtho,  stress::Vector{FT}, output::Vector{FT}, strain::Vector{FT}, thstrain::Vector{FT}=zeros(4), t::FT= 0.0, dt::FT= 0.0, loc::Matrix{FT}=zeros(3,1), label::Int=0, quantity=:nothing)
		@assert length(stress) == nstressstrain(self.mr)
		D = zeros(3, 3)
...
"""
function MatDeforElastOrtho(mr::Type{DeforModelRed2DAxisymm}, args::NTuple{13, FT}) where FT
    mass_density, E1, E2, E3, nu12, nu13, nu23, G12, G13, G23, CTE1, CTE2, CTE3 = args
	function tangentmoduli2daxi!(self::MatDeforElastOrtho, D::Matrix{FT}, t::FT, dt::FT, loc::Matrix{FT}, label::Int)
		for i = 1:4
			for j = 1:4
				D[i, j] = self.D[i, j];
			end
	...
	function update2daxi!(self::MatDeforElastOrtho,  stress::Vector{FT}, output::Vector{FT}, strain::Vector{FT}, thstrain::Vector{FT}=zeros(3), t::FT= 0.0, dt::FT= 0.0, loc::Matrix{FT}=zeros(3,1), label::Int=0, quantity=:nothing)
		@assert length(stress) == nstressstrain(self.mr)
		D = zeros(4, 4)
