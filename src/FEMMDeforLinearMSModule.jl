...
import ..DeforModelRedModule: AbstractDeforModelRed, DeforModelRed3D
import ..MatDeforLinearElasticModule: AbstractMatDeforLinearElastic, tangentmoduli!, update!, thermalstrain!
import ..MatDeforElastIsoModule: MatDeforElastIso
...
import ..DeforModelRedModule: nstressstrain, nthermstrain, Blmat!, divmat, vgradmat
...
import ..FEMMDeforLinearBaseModule: stiffness, nzebcloadsstiffness, mass, thermalstrainloads, inspectintegpoints
import FinEtools.FEMMBaseModule: associategeometry!
import ..MatDeforModule: rotstressvec!
...
"""
    infsup_gh(self::AbstractFEMMDeforLinearMS, assembler::A, geom::NodalField{FFlt}, u::NodalField{T}) where {A<:AbstractSysmatAssembler, T<:Number}
...
function infsup_gh(self::AbstractFEMMDeforLinearMS, geom::NodalField{FFlt},  u::NodalField{T}) where {T<:Number}
    assembler = SysmatAssemblerSparseSymm();
    return infsup_gh(self, assembler, geom, u);
end

"""
    infsup_sh(self::AbstractFEMMDeforLinearMS, assembler::A, geom::NodalField{FFlt}, u::NodalField{T}) where {A<:AbstractSysmatAssembler, T<:Number}
...
function infsup_sh(self::AbstractFEMMDeforLinearMS, geom::NodalField{FFlt},  u::NodalField{T}) where {T<:Number}
    assembler = SysmatAssemblerSparseSymm();
